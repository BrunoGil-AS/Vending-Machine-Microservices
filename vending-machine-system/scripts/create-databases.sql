...
DROP DATABASE IF EXISTS vending_config;
CREATE DATABASE vending_config;
DROP USER IF EXISTS 'service_config'@'localhost';
CREATE USER 'service_config'@'localhost' IDENTIFIED BY 'securePassword123';
GRANT SELECT, INSERT, UPDATE, DELETE, ALTER, DROP, REFERENCES, CREATE ON vending_config.* TO 'service_config'@'localhost';

-- Authentication Database
DROP DATABASE IF EXISTS vending_auth;
CREATE DATABASE vending_auth;
DROP USER IF EXISTS 'service_auth'@'localhost';
CREATE USER 'service_auth'@'localhost' IDENTIFIED BY 'securePassword123';
GRANT SELECT, INSERT, UPDATE, DELETE, ALTER, DROP, REFERENCES, CREATE ON vending_auth.* TO 'service_auth'@'localhost';
